...
function JSDialogDrawTool()
{
    ...
    this.ColumnCount=5
    this.ContinuousDrawData=null;   //连续画线当前工具
    ...
    this.Create=function()
    {
        ...
        this.OnContextMenu=(e)=>
        {
            ...
            if (this.ContinuousDrawData && this.ContinuousDrawData.Span && this.ContinuousDrawData.Span.classList.contains("UMyChart_DrawTool_Span_Selected"))
            {
                this.CreateDrawPicture(this.ContinuousDrawData);
            }
        };
    ...
    this.CreateDrawPicture=function(data)
    {
        ...
        if (name=="线段" && this.HQChart.CurrentChartDrawPicture) this.HQChart.CurrentChartDrawPicture.__LineClickMode=true;
    }
    ...
    //画图工具绘制完成 - 连续画线模式：自动开始下一条
    this.OnFinishDrawPicture=function(chart, data)
    {
        if (this.ContinuousDrawData!==data) return;
        if (!data || !data.Span || !data.Span.classList || !data.Span.classList.contains("UMyChart_DrawTool_Span_Selected")) return;
        this.CreateDrawPicture(data);
    }
